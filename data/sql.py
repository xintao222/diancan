...
def see():
    ...
    for i in all_froms:
        ...
        cu.execute(
            'select sum(o.price*o.number) from orders o where o.froms = "%s" and o.day = "%s"' %
            (froms, str_time))
        ...
        print "---------------"
        li = cu.execute(
            'select dish,sum(number) from orders where froms = "%s" and day = "%s" group by dish' %
            (froms, str_time))
        # li = cu.fetchall()
        print li
        # print len(li)
        ...
        for j in li:
            ...
            sql = 'select id from orders where day = "%s" and froms = "%s" and dish = "%s"' % (
                str_time, froms, dish)
            print sql
            cu.execute(sql)
            print cu.fetchall()
